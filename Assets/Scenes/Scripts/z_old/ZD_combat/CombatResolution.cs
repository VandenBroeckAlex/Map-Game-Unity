...
public class CombatResolution : MonoBehaviour
{
    ...
    public void CombatTurn()
    {
        ...
        for (int i = 0; i < attackerList.Count; i++)
        {
            ...
            //1) shot fire
            int attackerDamage = Convert.ToInt32(Math.Floor(attacker.firepower * (attacker.manPower.current / attacker.manPower.max) * (attacker.organisation.current / attacker.organisation.max)));
            int targetDamage = Convert.ToInt32(Math.Floor(target.firepower * (target.manPower.current / target.manPower.max) * (target.organisation.current / target.organisation.max)));
